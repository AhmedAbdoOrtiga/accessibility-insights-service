...
import { Selector, AxeCoreResults, AxeScanResultsHashable, AxeResult } from './axe-result-types';
...
export class AxeResultsReducer {
    ...
    private reduceResults(url: string, accumulatedResults: AxeScanResultsHashable, currentResults: axe.Result[]): void {
        if (currentResults) {
    ...
    private reduceResultsWithoutNodes(url: string, accumulatedResults: AxeScanResultsHashable, currentResults: axe.Result[]): void {
        if (currentResults) {
            for (const currentResult of currentResults) {
                if (currentResult) {
